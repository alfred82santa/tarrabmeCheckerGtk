...
class SetupDialog(Gtk.Dialog):
    def __init__(self, parent):
        Gtk.Dialog.__init__(self, "Setup", parent, Gtk.DialogFlags.MODAL,
        ...
        self.soundLeftAction = Gtk.RadioAction("sound-left", "Channel Left", 
        ...
        button.set_related_action(self.soundLeftAction)
        button.set_use_action_appearance(False)
        
        soundButtonsBox.pack_start(button, True, True, 0)
        
        self.soundStereoAction = Gtk.RadioAction("sound-stereo", "Stereo", 
            "Choose sound on stereo", None, value=1)
        self.soundStereoAction.join_group(self.soundLeftAction)
        
        button = Gtk.ToggleButton()
        icon = Gtk.Image.new_from_resource('/org/me/tarrab/Checker/sound-stereo.svg')
        button.add(icon)
        
        button.set_related_action(self.soundStereoAction)
        button.set_use_action_appearance(False)
        
        soundButtonsBox.pack_start(button, True, True, 0)
        
        self.soundRightAction = Gtk.RadioAction("sound-right", "Channel Right", 
            "Choose sound on channel rigth", None, value=2)
        self.soundRightAction.join_group(self.soundLeftAction)
        
        button = Gtk.ToggleButton()
        icon = Gtk.Image.new_from_resource('/org/me/tarrab/Checker/sound-right.svg')
        button.add(icon)
        
        button.set_related_action(self.soundRightAction)
        button.set_use_action_appearance(False)
    ...
    def _set_scanner(self, scanner_id = None):
        ...
        dev.grab(self.keyboard_test.get_window(), Gdk.GrabOwnership.NONE, True, Gdk.EventMask.ALL_EVENTS_MASK, None, Gdk.CURRENT_TIME)
