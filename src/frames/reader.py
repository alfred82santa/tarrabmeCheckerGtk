...
import os, sys
from gi.repository import WebKit, Gtk, Gdk, GObject, Soup, Pango, Gio
from .login import LoginDialog
from .setup import SetupDialog
from audio.pipes import make_notification_emitter
from widgets.combos import KeyboardCombo
from config import *
...
class Reader(Gtk.VBox):
    def __init__(self, parent, name='default', *args, **kwargs):
        Gtk.VBox.__init__(self, *args, **kwargs)
        self.parent = parent
        
        self.toolbar = Gtk.HeaderBar()
        self.toolbar.set_subtitle(name)
    ...
    def on_click_setup(self, button):
        ...
        try:
            self._load_config(dialog)
        except:
            print("Error loading dialog data")
    ...
    def set_settings(self, settings, index=1):
        self.general_setting = settings
        
        self.settings = Gio.Settings.new_with_path(self.CONFIG_SCHEMA,
            settings.get_property("path") + "/reader" + index)
        
    def _load_config(self, dialog):
        self.settings.delay()
        
        dialog.set_scanner_device(self._scanner_device)
        dialog.set_sound_device(self._sound_device)
    ...
    def on_scan_barcode(self, code):
        ...
        attempt_path = self.general_setting.get_string("attempt-path")
