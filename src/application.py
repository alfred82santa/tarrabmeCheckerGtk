...
from config import CONFIG_PATH, MEDIA_PATH
...
class TarrabmeCheckerApp(Gtk.Application):
    ...
    def __init__(self):
        ...
        if not self.get_is_remote():
            ...
            self.sss = Gio.SettingsSchemaSource.new_from_directory(CONFIG_PATH, None, False)
            schema = self.sss.lookup(self.CONFIG_SCHEMA, False)
